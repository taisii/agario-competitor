...
from strategies.replay_team_48 import ReplayTeam48Strategy  # noqa: E402
...
def _choose(strategy: ReplayTeam48Strategy, game: SimpleNamespace):
    return strategy.choose(StrategyContext(game=game, query=SimpleNamespace()))


def test_team_48_uses_all_sources_and_preserves_failed_gate() -> None:
    strategy = ReplayTeam48Strategy()

    assert strategy.profile.team_id == 48
    assert strategy.profile.source_matches == (11719, 11745, 11756)
    assert strategy.profile.validation_passed is False


def test_team_48_safe_initial_direction_is_food_attracted() -> None:
    decision = _choose(
        ReplayTeam48Strategy(),
...
def test_team_48_initial_predator_regime_escapes() -> None:
...
def test_team_48_safe_prey_overrides_opposed_food() -> None:
...
def test_team_48_cannot_split_below_engine_mass_threshold() -> None:
    prey = VisibleBlobModel(
        player_id=1,
        team_id=9,
        blob_id=0,
