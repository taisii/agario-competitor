...
from strategies.replay_team_49 import ReplayTeam49Strategy  # noqa: E402
...
def _choose(strategy: ReplayTeam49Strategy, game: SimpleNamespace):
    return strategy.choose(StrategyContext(game=game, query=SimpleNamespace()))


def test_team_49_uses_all_sources_and_preserves_failed_gate() -> None:
    strategy = ReplayTeam49Strategy()

    assert strategy.profile.team_id == 49
    assert strategy.profile.source_matches == (11654, 11697, 11745, 11756, 13933)
    assert strategy.profile.validation_passed is False


def test_team_49_safe_initial_direction_is_food_attracted() -> None:
    decision = _choose(
        ReplayTeam49Strategy(),
...
def test_team_49_initial_predator_regime_escapes() -> None:
...
def test_team_49_safe_prey_overrides_opposed_food() -> None:
...
def test_team_49_cannot_split_below_engine_mass_threshold() -> None:
    prey = VisibleBlobModel(
        player_id=2,
        team_id=9,
        blob_id=0,
