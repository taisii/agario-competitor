...
- 相手は `bots/entries/replay_opponent.py` から毎試合7体。
...
.venv/bin/python scripts/benchmark_simulations.py \
  ...
  --jobs 4 \
  --variants current \
  --submission 1:dist/my_bot.py 7:bots/entries/replay_opponent.py \
  --tracked-slots 0 \
  --metrics-every-n 100 \
...
評価用に `bots/entries/replay_opponent.py` を追加した。公式30試合の7-team
